...
import { ShieldCheck, Lock, ArrowLeft, BookOpen, PenTool, CheckCircle2, ChevronRight, Menu } from 'lucide-react';
...
import { LEGAL_DICTIONARY_DB, highlightGlossaryTerms } from '../services/glossary';
...
const TiptapRenderer: React.FC<{ 
    ...
    const processedNodes = useMemo(() => {
        ...
        const processNode = (domNode: Node, index: number): React.ReactNode => {
            if (domNode.nodeType === Node.ELEMENT_NODE) {
                ...
                if (element.tagName.toLowerCase() === 'hypr-block') {
                    ...
                    if (blockData) {
                        try {
                            ...
                            return <React.Fragment key={`block-${block.id}-${index}`}>{renderBlock(block)}</React.Fragment>;
                        } catch (e) {
                            return <div key={index} className="text-red-500 text-xs">Error parsing block</div>;
                        }
                    }
                }
                ...
                if (element.getAttribute('data-type') === 'taskItem') {
                     ...
                     return (
                         <li key={index} className="flex items-center gap-2">
                             ...
                             <span>{Array.from(element.childNodes).map((child, i) => processNode(child, i))}</span>
                         </li>
                     )
                }
                ...
                const children = Array.from(element.childNodes).map((child, i) => processNode(child, i));
                ...
                if (!hasElementChildren && element.textContent && element.textContent.trim().length > 0 && !element.classList.contains('glossary-term')) {
                     const highlightedHtml = highlightGlossaryTerms(element.innerHTML, terms);
    ...
    return <div className="tiptap prose dark:prose-invert max-w-none prose-p:my-2 prose-headings:my-4">{processedNodes}</div>;
};


// --- Main Viewer Component ---

export const Viewer: React.FC<ViewerProps> = ({ blocks, snapshot, parties = [], variables = [], terms = [], isPreview = false, settings, docHash, onSigningComplete, status, verifiedIdentifier, contentHtml, onExitPreview, docId: propDocId }) => {
  ...
  const actionItems = useMemo(() => {
      return allBlocksFlat.filter(b => b.type === BlockType.SIGNATURE && b.assignedToPartyId === simulatedPartyId);
  ...
  const activeDefinitions = useMemo(() => {
      const allDefinitions = [...LEGAL_DICTIONARY_DB, ...terms.map(t => ({ term: t.term, definition: t.definition, category: 'Custom' }))];
      const uniqueFound = Array.from(new Set(foundTerms));
      
      return uniqueFound.map((term: string) => {
          const def = allDefinitions.find(d => d.term.toLowerCase() === term.toLowerCase());
  ...
  return (
    <div className="flex bg-muted/10 bg-grid-pattern min-h-screen relative font-sans">
        ...
        {(isPreview || !isCompleted) && (
            <div className={cn("fixed left-0 top-20 bottom-0 w-80 bg-white dark:bg-black border-r-2 border-black dark:border-white z-40 transition-transform duration-300 transform", isGuideOpen ? "translate-x-0" : "-translate-x-80")}>
                <button 
                ...
                >
                    {isGuideOpen ? <ChevronRight className="rotate-180" size={16}/> : <BookOpen size={16}/>}
                </button>
                ...
                <div className="flex flex-col h-full overflow-hidden">
                    ...
                    <div className="p-4 space-y-3 overflow-y-auto max-h-[40%] border-b-2 border-black/10 dark:border-white/10">
                        {actionItems.length === 0 ? (
                            <div className="text-xs text-muted-foreground italic">No signatures required for this party.</div>
                        ) : (
                            actionItems.map(b => (
                                <div key={b.id} className="flex items-center gap-2 text-xs p-2 border bg-card cursor-pointer hover:bg-black hover:text-white transition-colors" onClick={() => {
                                    document.getElementById(b.id)?.scrollIntoView({ behavior: 'smooth', block: 'center' });
                                    setActiveFieldId(b.id);
                                }}>
                                    {formValues[b.id] ? <CheckCircle2 size={14} className="text-green-500" /> : <div className="w-3.5 h-3.5 border rounded-full" />}
                    ...
                    <div className="p-4 space-y-4 overflow-y-auto flex-1">
                        {activeDefinitions.length === 0 ? (
                            <div className="text-xs text-muted-foreground italic">No glossary terms found in document text.</div>
                        ) : (
                            activeDefinitions.map((d, i) => (
                                <div key={i} className="text-xs space-y-1">
                                    <div className="font-bold text-primary border-b border-dashed border-primary inline-block">{d.term}</div>
        ...
        <div className={cn("flex-1 transition-all duration-300", isGuideOpen && (isPreview || !isCompleted) ? "ml-80" : "ml-0")}>
            {/* Completion Overlay */}
            {isCompleted && (
            ...
            {isPreview && (
                <div className="sticky top-0 z-50 bg-white/95 dark:bg-black/95 backdrop-blur-md border-b-2 border-black dark:border-white px-8 py-4 flex justify-between items-center shadow-lg h-20">
                    <div className="flex items-center gap-4">
                        <Button variant="outline" onClick={onExitPreview} className="gap-2 font-mono h-8 border-black hover:bg-black hover:text-white dark:border-white dark:hover:bg-white dark:hover:text-black">
                            <ArrowLeft size={16} /> BACK TO EDITOR
                        </Button>
                        <div className="h-8 w-px bg-black/10 dark:bg-white/10 mx-2" />
                        <div className="flex items-center gap-2 text-sm text-muted-foreground">
                            <span className="font-mono text-[10px] uppercase font-bold tracking-wider">Viewing as:</span>
                                ...
                                {parties.map(p => (
                                    <button 
                                        ...
                                        className={cn(
                                            ...
                                            simulatedPartyId === p.id ? "shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] -translate-y-[1px]" : "bg-transparent hover:bg-black/5 text-muted-foreground opacity-50 hover:opacity-100"
                                        )}
                                        style={{ 
                                            backgroundColor: simulatedPartyId === p.id ? p.color : 'transparent',
                                            color: simulatedPartyId === p.id ? getContrastColor(p.color) : undefined,
                                            borderColor: simulatedPartyId === p.id ? 'black' : undefined
                                        }}
                                    >
                                        {p.name}
                                    </button>
            ...
            <div 
                className={cn("max-w-[850px] mx-auto bg-white dark:bg-black border-2 border-black dark:border-zinc-800 shadow-hypr dark:shadow-hypr-dark relative transition-all p-16 min-h-[1100px]", isPreview ? "mt-8 mb-24 pt-32" : "mt-8")}
            ...
            >
                ...
                <div className="space-y-6">
                    {contentHtml ? (
                        <TiptapRenderer html={contentHtml} renderBlock={renderBlockCallback} terms={terms} onTermsFound={setFoundTerms} />
                    ) : (
