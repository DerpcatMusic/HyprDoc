...
import { ArrowRight, CheckCircle2, ChevronRight, CornerDownLeft, FastForward, Navigation } from 'lucide-react';
...
export const WizardBar: React.FC<WizardBarProps> = ({ allBlocks, formValues, onChange, activePartyId, onFocus, isCompleted, activeFieldId }) => {
    
    // Calculate required fields logic
    const { nextField, progress, totalRequired, completedRequired } = useMemo(() => {
        const requiredFields = allBlocks.filter(b => 
            b.required && 
            !['text','alert','spacer','column','columns','section_break'].includes(b.type) &&
        ...
        return {
            nextField: next,
            progress: requiredFields.length > 0 ? (completed.length / requiredFields.length) * 100 : 100,
        ...
        <div className="fixed bottom-0 left-0 right-0 z-50 p-4 pb-6 pointer-events-none">
            <div className="max-w-2xl mx-auto bg-black dark:bg-zinc-900 text-white shadow-2xl border-2 border-white/20 pointer-events-auto flex flex-col overflow-hidden animate-in slide-in-from-bottom duration-300">
                ...
                <div className="flex items-center p-3 gap-4">
                     ...
                     <div className="flex-1 flex flex-col justify-center min-w-0">
                         {nextField ? (
                             <>
                                <div className="flex items-center justify-between mb-1">
                                    <span className="text-[9px] font-mono font-bold uppercase tracking-widest text-primary flex items-center gap-2">
                                        <Navigation size={10} /> FIELD {completedRequired + 1} OF {totalRequired}
                                    </span>
                                    <span className="text-[9px] text-white/50 font-mono uppercase">Enter to Next</span>
                                </div>
                                
                                <div className="flex items-center gap-3">
                                    <span className="text-sm font-bold truncate whitespace-nowrap opacity-70">{nextField.label || 'Input'}:</span>
                                    ...
                                    {['input', 'email', 'number'].includes(nextField.type) ? (
                                        ...
                                        <div className="text-xs text-white/50 italic flex-1 cursor-pointer hover:text-white" onClick={() => onFocus(nextField.id)}>
                             ...
                             <div className="flex items-center gap-3 text-green-400">
                                 ...
                                 <div>
                                     <div className="font-bold uppercase tracking-wide text-xs">All Fields Complete</div>
                                     <div className="text-[10px] opacity-70">You can now finalize the document.</div>
                                 </div>
                             </div>
                         )}
                     </div>
                     ...
                     <Button 
                        size="icon" 
                        className={cn("h-12 w-12 rounded-none shrink-0 border-2", nextField ? "bg-white text-black hover:bg-primary border-transparent" : "bg-green-500 text-black border-green-400 animate-pulse")}
                        onClick={() => {
                            if (nextField) onFocus(nextField.id);
                        }}
                     >
                         {nextField ? <CornerDownLeft size={20} /> : <CheckCircle2 size={24} />}
                     </Button>
                </div>
            </div>
        </div>
