...
export const InlineTiptapEditor = ({ 
    ...
    useEffect(() => {
        if (editor && content !== undefined && editor.getHTML() !== content) {
            ...
            if (Math.abs(editor.getHTML().length - (content || '').length) > 5 || !content) { 
...
export const TextEditor: React.FC<EditorBlockProps> = (props) => {
    ...
    const menuStateRef = useRef({ showSlashMenu: false, slashFilter: '', slashMenuIndex: 0 });
    useEffect(() => {
    ...
    const handleSlashSelect = (type: string, view?: any) => {
        ...
        if (['h1', 'h2', 'bulletList'].includes(type)) {
             ...
             if (type === 'h1') editor.view.dispatch(tr.setBlockType(from, from, editor.state.schema.nodes.heading, { level: 1 }));
             else if (type === 'h2') editor.view.dispatch(tr.setBlockType(from, from, editor.state.schema.nodes.heading, { level: 2 }));
    ...
    const editor = useEditor({
        ...
        editorProps: {
            handleKeyDown: (view, event) => {
                const { showSlashMenu, slashFilter, slashMenuIndex } = menuStateRef.current;
                ...
                if (showSlashMenu) {
                    ...
                    if (event.key === 'Enter') {
                        ...
                        if (filtered[slashMenuIndex]) handleSlashSelect(filtered[slashMenuIndex].type, view);
                        return true;
                    }
                    if (event.key === 'Escape') {
                        event.preventDefault();
    ...
    return (
        <div ref={elementRef}
             className={cn("relative group mb-3 transition-all", isSelected ? "z-20" : "")}
             onClick={(e) => { e.stopPropagation(); onSelect(block.id); }}
             {...containerProps}
        >
            {dropPosition && !isTiptap && (
                <div className={cn("absolute left-0 right-0 h-1 bg-primary z-50 pointer-events-none shadow-[0_0_10px_rgba(var(--primary),0.5)]", dropPosition === 'before' ? "-top-2" : "-bottom-2")} />
            )}

            {/* Drag Handle - Use data-drag-handle for Tiptap */}
            <div className={cn("absolute -left-6 top-1 cursor-grab opacity-0 group-hover:opacity-100 transition-opacity text-muted-foreground hover:text-foreground", isSelected ? "opacity-100" : "")}
                 // IMPORTANT: Do NOT stopPropagation if Tiptap needs to handle the drag event
                 onMouseDown={(e) => { if(!isTiptap) e.stopPropagation(); }} 
            ...
            {/* Content */}
            <div className={cn("min-h-[2.5em] relative border border-transparent rounded-sm", isSelected ? "ring-2 ring-primary ring-offset-2" : "hover:border-black/10 dark:hover:border-white/10")}>
                
                {editor && (
                    <BubbleMenu editor={editor} tippyOptions={{ duration: 100 }} className="flex items-center gap-1 p-1 bg-black text-white rounded-sm shadow-xl border border-white/20">
                        <button onClick={() => editor.chain().focus().toggleBold().run()} className={cn("p-1 hover:bg-white/20 rounded", editor.isActive('bold') ? 'bg-white/30 text-white' : '')}><Bold size={14}/></button>
                        <button onClick={() => editor.chain().focus().toggleItalic().run()} className={cn("p-1 hover:bg-white/20 rounded", editor.isActive('italic') ? 'bg-white/30 text-white' : '')}><Italic size={14}/></button>
                        <button onClick={() => editor.chain().focus().toggleStrike().run()} className={cn("p-1 hover:bg-white/20 rounded", editor.isActive('strike') ? 'bg-white/30 text-white' : '')}><Strikethrough size={14}/></button>
                        <div className="w-px h-4 bg-white/20 mx-1" />
                        <button onClick={() => editor.chain().focus().toggleHeading({ level: 1 }).run()} className={cn("p-1 hover:bg-white/20 rounded", editor.isActive('heading', { level: 1 }) ? 'bg-white/30 text-white' : '')}><Heading1 size={14}/></button>
                        <button onClick={() => editor.chain().focus().toggleHeading({ level: 2 }).run()} className={cn("p-1 hover:bg-white/20 rounded", editor.isActive('heading', { level: 2 }) ? 'bg-white/30 text-white' : '')}><Heading2 size={14}/></button>
                        <button onClick={() => editor.chain().focus().toggleBulletList().run()} className={cn("p-1 hover:bg-white/20 rounded", editor.isActive('bulletList') ? 'bg-white/30 text-white' : '')}><List size={14}/></button>
                        <div className="w-px h-4 bg-white/20 mx-1" />
                        <button onClick={() => setShowMagicMenu(!showMagicMenu)} className="p-1 hover:bg-purple-600 text-purple-300 hover:text-white rounded flex items-center gap-1">
                            <Sparkles size={14} /> AI
                        </button>
                    </BubbleMenu>
                )}
                ...
                {showMagicMenu && (
                     <div className="absolute top-full left-0 z-50 bg-black text-white p-2 rounded-sm shadow-xl flex flex-col gap-1 w-48 mt-2 animate-in fade-in zoom-in-95">
                         <div className="text-[9px] font-bold uppercase tracking-widest opacity-50 mb-1 px-2">AI Refinement</div>
                         <button onClick={handleGenerate} disabled={isRefining} className="text-left text-xs px-2 py-1.5 hover:bg-white/20 rounded-sm flex items-center gap-2">
                             <Wand2 size={12}/> Make Legalese
                         </button>
                         <button onClick={() => {}} disabled className="text-left text-xs px-2 py-1.5 hover:bg-white/20 rounded-sm opacity-50 cursor-not-allowed">
                             Fix Grammar (Soon)
                         </button>
                     </div>
                )}
            ...
            {isSelected && (
                <div className="absolute -right-8 top-0 flex flex-col gap-1">
                     <button className="p-1.5 bg-white border shadow-sm hover:text-red-500 rounded-sm" onClick={() => onDelete(block.id)}><Trash2 size={14} /></button>
                </div>
            )}
        </div>
    );
