...
const DropIndicator = ({ position }: { position: string | null }) => {
  if (position === "before")
    return (
      <div className="absolute -top-2 left-0 right-0 h-1 bg-primary z-50 pointer-events-none shadow-[0_0_10px_rgba(var(--primary),0.5)]" />
    );
  if (position === "after")
    return (
      <div className="absolute -bottom-2 left-0 right-0 h-1 bg-primary z-50 pointer-events-none shadow-[0_0_10px_rgba(var(--primary),0.5)]" />
    );
  return null;
};
...
const SelectedHeader = ({
  ...
  <div className="absolute -top-3 right-0 flex border-2 border-black dark:border-white bg-white dark:bg-black shadow-sharp z-50 h-6">
    <div className="px-2 flex items-center bg-black dark:bg-white text-white dark:text-black text-[9px] font-bold font-mono uppercase">
      {label}
    </div>
    <button
      className="px-2 hover:bg-red-600 hover:text-white dark:text-white transition-colors"
      onClick={(e) => {
        e.stopPropagation();
        onDelete();
      }}
...
const ColumnsEditor: React.FC<EditorBlockProps> = (props) => {
  ...
  return (
    <div
      className={cn(
        "w-full relative mb-6 group border-2 border-transparent transition-all",
        isSelected
          ? "border-primary/50 bg-primary/5 p-2 rounded-sm"
          : "hover:border-black/10 p-2"
      )}
      onClick={(e) => {
        e.stopPropagation();
        onSelect(block.id);
      ...
      >
        ...
        >
          ...
          <button
            ...
            className="text-[9px] font-mono hover:text-red-500 uppercase flex items-center gap-1 border border-black/10 px-1 bg-white"
          >
        ...
        <div className="flex items-center gap-1">
          <button
            ...
            className="w-5 h-5 flex items-center justify-center border border-black/20 bg-white hover:bg-black hover:text-white disabled:opacity-30 transition-colors"
          >
            <Minus size={10} />
          </button>
...
const ColumnDropZone: React.FC<EditorBlockProps> = (props) => {
  const { block } = props;
  return (
    <div className="flex-1 flex flex-col gap-3 relative min-h-[100px] p-2 border border-dashed border-black/10 bg-white dark:bg-black/20 transition-colors">
...
const SpacerEditor: React.FC<EditorBlockProps> = (props) => {
  ...
  return (
    <div
      ...
      className={cn(
        "group relative flex items-center justify-center cursor-ns-resize",
        isSelected ? "border border-primary border-dashed" : "hover:bg-muted/10"
      )}
      style={{ height: Math.max(10, block.height || 32) }}
      {...{ "data-drag-handle": "" }}
    >
      <div
        className="absolute left-2 top-1/2 -translate-y-1/2 bg-white border border-black/10 p-1 rounded-sm opacity-0 group-hover:opacity-100 shadow-sm cursor-pointer z-10"
        onMouseDown={(e) => {
          e.stopPropagation();
      ...
      <span className="text-[9px] text-muted-foreground opacity-0 group-hover:opacity-100 select-none">
        {block.height || 32}px
      </span>
      {isSelected && (
        <SelectedHeader label="SPACER" onDelete={() => onDelete(block.id)} />
      )}
    </div>
  );
};
...
const AlertEditor: React.FC<EditorBlockProps> = (props) => {
  ...
  return (
    <div
      className={cn("relative group mb-3", isSelected && "z-20")}
      onClick={(e) => {
        e.stopPropagation();
        onSelect(block.id);
      }}
    >
      {isSelected && (
        <SelectedHeader label="ALERT" onDelete={() => onDelete(block.id)} />
      )}

      {/* The container is the drag handle for alert boxes */}
      <div
        className={cn(
          "p-4 border-l-4 flex gap-3 items-start cursor-grab active:cursor-grabbing",
          styles[variant],
        ...
        <div className="flex flex-col gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
          {["info", "warning", "error", "success"].map((v) => (
            <button
              ...
              onClick={(e) => {
                e.stopPropagation();
                onUpdate(block.id, { variant: v as any });
              }}
              className={cn(
...
const QuoteEditor: React.FC<EditorBlockProps> = (props) => {
  ...
  return (
    <div
      className={cn(
        "relative group mb-3 pl-4 border-l-4 border-black/20 dark:border-white/20",
    ...
    >
      <div className="relative">
        {/* Icon is drag handle */}
        <div
          className="absolute -top-2 -left-2 text-muted-foreground/20 -z-10 transform -scale-x-100 cursor-grab active:cursor-grabbing"
          {...{ "data-drag-handle": "" }}
        >
          <Quote size={24} />
        </div>
      ...
      {isSelected && (
        <div className="absolute top-0 right-0">
          <button
            className="p-1 hover:bg-red-600 hover:text-white text-muted-foreground transition-colors"
            onClick={(e) => {
              e.stopPropagation();
              onDelete(block.id);
            }}
...
const RepeaterEditor: React.FC<EditorBlockProps> = (props) => {
  ...
  return (
    <div
      className={cn(
        "relative mb-6 border-2 bg-white dark:bg-black p-4",
        isSelected ? "border-primary shadow-lg" : "border-black border-dashed"
      )}
      onClick={(e) => {
        e.stopPropagation();
        onSelect(block.id);
    ...
    >
      ...
      <div className="min-h-[60px] space-y-4">
        {!block.children?.length && (
          <div className="text-center text-[10px] text-muted-foreground uppercase py-4">
            Drop content here
          </div>
        )}
        {block.children?.map((child, i) => (
...
const StandardEditor: React.FC<EditorBlockProps> = (props) => {
  ...
  const handlePartyToggle = (e: React.MouseEvent) => {
    e.stopPropagation();
    const currentIndex = parties.findIndex(
      (p) => p.id === block.assignedToPartyId
    );
    const nextParty =
      currentIndex === -1 ? parties[0] : parties[currentIndex + 1];
  ...
  return (
    ...
    >
      ...
      <div
        className={cn(
          "relative border-2 transition-all bg-white dark:bg-black p-0 overflow-hidden",
          isSelected
            ? "border-primary shadow-sharp-hover ring-1 ring-primary"
            : "border-black/10 dark:border-white/20 hover:border-black/40"
        )}
      >
        ...
        {/* Header acts as drag handle - hover shows draggability, active shows grab state */}
        <div
          className="flex items-center justify-between px-3 py-2 bg-muted/5 border-b border-black/5 select-none cursor-grab active:cursor-grabbing group/handle transition-all hover:bg-primary/5 hover:border-primary/20 active:bg-primary/10 active:ring-2 active:ring-primary/30 active:ring-offset-1"
          {...{ "data-drag-handle": "" }}
        >
          <div className="flex items-center gap-3 pl-1">
            ...
            <GripVertical
              size={12}
              className="opacity-0 group-hover/handle:opacity-50 transition-opacity text-muted-foreground -ml-2 mr-0"
            />
            <Icon size={14} className="text-muted-foreground" />
            <span className="text-[10px] font-mono font-bold uppercase tracking-wider text-muted-foreground group-hover/handle:text-primary/70 transition-colors">
          ...
          {!isLayoutBlock && (
            ...
            >
              <button
                ...
                className={cn(
                  "h-5 px-2 text-[9px] font-bold uppercase border flex items-center gap-1 transition-all",
                  assignedParty
                    ? "bg-white border-transparent shadow-sm"
                    : "bg-transparent border-transparent text-muted-foreground/50 hover:bg-muted"
                )}
                title={
                  assignedParty
                    ? `Assigned to ${assignedParty.name}`
                    : "Click to assign party"
                }
              >
                {assignedParty ? (
                  <>
              ...
              <button
                ...
                className={cn(
                  "h-5 px-2 text-[9px] font-bold uppercase border transition-all flex items-center gap-1",
                  block.required
                    ? "bg-red-50 text-red-600 border-red-200"
                    : "text-muted-foreground/50 border-transparent hover:bg-muted"
                )}
        ...
        <div className="p-3 pl-4">
          {/* Visual Previews */}
          {block.label && (
            <div className="text-[10px] font-bold font-mono uppercase tracking-wider text-muted-foreground mb-1.5">
              {block.label}
            </div>
          )}
          ...
          {block.type === BlockType.VIDEO && (
            <div className="h-20 bg-black/5 flex items-center justify-center gap-2 text-xs text-muted-foreground">
              ...
              <span>{block.videoUrl || "Video Embed"}</span>
            </div>
          )}
          ...
          {["input", "number", "email", "long_text"].includes(block.type) && (
            <div
              className={cn(
                "bg-muted/10 border-b-2 border-black/10 w-full flex items-center px-2 text-xs text-muted-foreground",
                block.type === "long_text" ? "h-16 items-start py-2" : "h-8"
              )}
            >
              {block.placeholder ||
          ...
          {block.type === BlockType.DATE && (
            <div className="h-8 bg-muted/10 border-b-2 border-black/10 w-full flex items-center px-2 justify-between">
              <span className="text-xs text-muted-foreground">
                {block.isDateRange ? "Start Date - End Date" : "DD / MM / YYYY"}
              </span>
          ...
          {block.type === BlockType.SELECT && (
            <div className="h-9 border-2 border-black/10 bg-white dark:bg-black w-full flex items-center justify-between px-3">
              <span className="text-xs text-muted-foreground italic">
                {(block.options && block.options[0]) || "Select option..."}
              </span>
          ...
          {block.type === BlockType.SECTION_BREAK && (
            <hr className="border-t-2 border-black/10 my-2" />
          )}
          ...
          {block.type === BlockType.FILE_UPLOAD && (
            <div className="h-12 bg-muted/5 border-2 border-dashed border-black/10 flex items-center justify-center gap-2 text-xs text-muted-foreground">
              <FileUp size={16} className="opacity-50" />{" "}
...
export const EditorBlock: React.FC<ExtendedEditorBlockProps> = (props) => {
  ...
  switch (block.type) {
    case BlockType.COLUMNS:
      return <ColumnsEditor {...props} />;
    case BlockType.COLUMN:
      return <ColumnDropZone {...props} />;
    case BlockType.SPACER:
      return <SpacerEditor {...props} />;
    case BlockType.ALERT:
      return <AlertEditor {...props} />;
    case BlockType.QUOTE:
      return <QuoteEditor {...props} />;
    case BlockType.REPEATER:
      return <RepeaterEditor {...props} />;
    case BlockType.TEXT:
      return <TextEditor {...props} />;
    case BlockType.PAYMENT:
      return <PaymentEditor {...props} />;
    case BlockType.CONDITIONAL:
      return <ConditionalZone {...props} />;
    default:
