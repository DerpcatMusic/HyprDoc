  ...
  const resize = useCallback(
    (mouseMoveEvent: MouseEvent) => {
      if (isResizing) {
        let newWidth;
        if (direction === "right") {
          newWidth = mouseMoveEvent.clientX - 64; // Minus nav width
        } else {
...
const AppContent: React.FC = () => {
  ...
  const [recipientIdentifier, setRecipientIdentifier] = useState<string | null>(
    null
  );
  const [isDarkMode, setIsDarkMode] = useState(() => {
    if (typeof window !== "undefined") {
      const saved = localStorage.getItem("hyprdoc-dark-mode");
  ...
  // Resizable States
  const leftPanel = useResizable(220, 180, 400, "right");
  const rightPanel = useResizable(340, 300, 500, "left");
  ...
  useEffect(() => {
    if (isDarkMode) {
      ...
      localStorage.setItem("hyprdoc-dark-mode", "true");
    } else {
      document.documentElement.classList.remove("dark");
      localStorage.setItem("hyprdoc-dark-mode", "false");
    }
  }, [isDarkMode]);
    ...
    if (!recipientIdentifier) {
      return (
        <AccessGate
          ...
          onAccessGranted={(id) => setRecipientIdentifier(id)}
        />
      );
    }
    return (
  ...
  if (rootRoute === "doc" && subView === "preview") {
    return (
      <div className="min-h-screen bg-background transition-colors font-sans">
        {/* Header is now managed by Viewer for consistency, but if we need app-level override: */}
        {/* Note: Viewer now handles its own header if isPreview is true, including the "Back" button */}
        <Viewer
          blocks={doc.blocks}
          settings={doc.settings}
          parties={doc.parties}
          ...
          docHash={doc.sha256}
          contentHtml={doc.contentHtml}
          onExitPreview={() => navigate(`#doc/${docId}/edit`)}
        />
      </div>
    );
  }
    ...
    <div className="flex h-screen w-screen bg-background text-foreground font-sans overflow-hidden relative">
      ...
      <div className="md:hidden absolute top-0 left-0 right-0 h-14 bg-background border-b-2 border-black flex items-center justify-between px-4 z-50">
        <div className="flex items-center gap-2">
          ...
          <span className="font-bold font-mono tracking-tight uppercase">
            HyprDoc
          </span>
        </div>
      ...
      >
        ...
        <div className="flex-1 flex flex-col gap-6 w-full px-2 items-center mt-4">
          ...
          >
            ...
            <span className="md:hidden ml-2 font-mono font-bold">
              Dashboard
            </span>
          </button>
      ...
      <div className="flex-1 flex flex-col min-w-0 h-full pt-14 md:pt-0">
        ...
        {rootRoute === "doc" && subView === "edit" && (
          <div className="flex flex-1 h-full relative overflow-hidden">
            {/* LEFT: TOOLBOX (Resizable) */}
            <div
              className="hidden md:flex bg-background flex-col z-10 shadow-none flex-shrink-0 relative group/handle"
            ...
            >
              ...
              {/* Drag Handle Right */}
              <div
                className="absolute right-0 top-0 bottom-0 w-1 bg-black/10 hover:bg-primary cursor-col-resize z-50 flex items-center justify-center transition-colors"
                onMouseDown={leftPanel.startResizing}
              >
                <div className="h-4 w-[2px] bg-current opacity-0 group-hover/handle:opacity-100" />
              </div>
            </div>

            {/* CENTER: EDITOR CANVAS */}
            <div className="flex-1 flex min-w-0 relative bg-muted/10 h-full overflow-hidden z-0">
              <EditorCanvas
                docTitle={doc.title}
            ...
            <div
              className="flex-shrink-0 z-10 h-full hidden lg:block relative group/handle"
              style={{ width: rightPanel.width }}
            >
              {/* Drag Handle Left */}
              <div
                className="absolute left-0 top-0 bottom-0 w-1 bg-black/10 hover:bg-primary cursor-col-resize z-50 flex items-center justify-center transition-colors"
        ...
        <DialogContent>
          ...
          <div className="p-8 bg-muted/10 border-2 border-dashed border-black dark:border-white text-sm text-center">
            <p className="font-mono text-xs mb-6 bg-white dark:bg-black border-2 border-black dark:border-white p-4 select-all shadow-sharp flex justify-between items-center">
              ...
              <span className="text-[9px] bg-black text-white px-2 py-1 font-bold cursor-pointer hover:bg-primary dark:bg-white dark:text-black">
                COPY
              </span>
            </p>
