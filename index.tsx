...
const DEFAULT_CONVEX_URL = "https://aromatic-lyrebird-949.convex.cloud";
const CLERK_PUBLISHABLE_KEY = getEnv(
  "CLERK_PUBLISHABLE_KEY",
  "VITE_CLERK_PUBLISHABLE_KEY",
  "pk_test_ZWxlZ2FudC1ncm91cGVyLTY1LmNsZXJrLmFjY291bnRzLmRldiQ"
);
let CONVEX_URL_RAW = getEnv(
  "CONVEX_URL",
  "VITE_CONVEX_URL",
  DEFAULT_CONVEX_URL
);
...
try {
  ...
  if (!CONVEX_URL_RAW || !CONVEX_URL_RAW.startsWith("http")) {
    console.warn(
      `Invalid CONVEX_URL "${CONVEX_URL_RAW}". Defaulting to Offline Mode.`
    );
  ...
  console.error(
    "CRITICAL: Convex Client Init Failed. App will run in Offline Mode.",
    e
  );
...
const ConvexClientProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  // If client is missing, render children without ConvexProvider
  if (!convexClient) {
    return <>{children}</>;
  }
